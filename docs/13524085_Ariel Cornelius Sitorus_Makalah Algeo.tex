...
\textsuperscript{1}\href{mailto:13524085@std.stei.itb.ac.id}{13524085@std.stei.itb.ac.id}, \textsuperscript{2}\href{mailto:arielsitorus504@gmail.com}{arielsitorus504@gmail.com}}
}
...
\subsubsection{Worked Example: 2D Eigendecomposition}
Consider a simple 2D covariance matrix:
\begin{equation}
...
\begin{equation}
    \det(\Sigma - \lambda I) = \det\begin{bmatrix} 5-\lambda & 3 \\ 3 & 5-\lambda \end{bmatrix} = 0
\end{equation}

Expanding:
\begin{equation}
    (5-\lambda)^2 - 9 = 0 \implies \lambda^2 - 10\lambda + 16 = 0
\end{equation}
...
\begin{equation}
    \begin{bmatrix} -3 & 3 \\ 3 & -3 \end{bmatrix} \begin{bmatrix} v_1 \\ v_2 \end{bmatrix} = 0 \implies v_1 = v_2
\end{equation}
...
\begin{enumerate}
    \item Center the data: $\bar{X} = X - \boldsymbol{\mu}$
    \item Compute covariance: $\Sigma = \frac{1}{N-1}\bar{X}^T\bar{X}$
    \item Eigendecompose: $\Sigma = Q\Lambda Q^T$
    \item Project: $Y = \bar{X}Q_k$ where $Q_k$ contains the top $k$ eigenvectors
\end{enumerate}
...
\subsection{Eigenvalue Interpretation in Physical Context}

The eigenvalues carry direct physical meaning in drone analysis:

\begin{itemize}
    \item $\lambda_1 = 96.34$: Represents variance along the primary flight direction. The large value indicates significant displacement in this direction over time.
    \item $\lambda_2 = 8.56$: Represents lateral drift or slight deviations from the main trajectory.
    \item $\lambda_3 = 1.01$: Represents sensor noise and minor vertical fluctuations.
\end{itemize}
...
\subsection{Analysis of Degenerate Cases}

A critical finding of this study is the detection of \textbf{eigenvalue degeneracy} in spiral trajectories. When $\lambda_1 \approx \lambda_2$, the eigenvectors are ill-defined in the subspace spanned by those eigenvalues.

For the spiral trajectory, we observed:
\begin{itemize}
    \item $\lambda_1/\lambda_2 = 1.64$ (near-degenerate)
    \item This indicates roughly equal variance in two perpendicular horizontal directions
    \item Geometrically, this corresponds to circular symmetry in the XY-plane
\end{itemize}
...
\subsection{Relationship to Singular Value Decomposition}
...
\begin{equation}
    \text{Cov}(\bar{X}) = \frac{1}{N-1}\bar{X}^T\bar{X} = \frac{1}{N-1}V\Sigma^2 V^T
\end{equation}
